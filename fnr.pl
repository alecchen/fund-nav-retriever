...
has 'company_links'      => ( is => 'rw', isa => 'ArrayRef', default => sub { [] } );
has 'company_names'      => ( is => 'rw', isa => 'ArrayRef', default => sub { [] } );
...
sub new {
    ...
    # fetch links and names
    my @url = ('http://www.funddj.com/y/yb/YP303000.djhtm', 'http://www.funddj.com/y/yb/YP303001.djhtm');
...
sub on_setup_input {
    ...
    my @company_names = @{ $self->company_names };
...
sub retrieve_nav {
    ...
    my $mech = WWW::Mechanize->new;
    my @company_links = @{ $self->company_links };
    ...
    foreach my $company_link (@company_links) {
        ...
        my @fund_links = grep { $_->url_abs =~ /yp01000[0-1]/ } $mech->links;
